...
fn build_listener() {
    ...
    let output = cmd.output().expect("Failed to execute cargo build for listener");
    ...
    let exe_name = if cfg!(target_os = "windows") {
        "horologion-listener.exe"
    } else {
        "horologion-listener"
    };
    
    // 根据 Tauri sidecar 命名规范生成目标文件名
    let target_exe_name = format!("listener-{}{}", 
        target_triple,
    ...
    println!("Successfully built and copied listener to {:?}", target_path);
}
